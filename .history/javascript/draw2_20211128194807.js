...
function drawGraph2(data){
    ...
    if(data != null){
        ...
        clearGraph();
        //parseData(data);
        var numLevels = 1 + Math.floor(Math.log2(test.length));
        ...
        //print(list);
        //print(test);
        print(printOddNodes(data,true,0,1));
...
function printOddNodes(root, isOdd,counter,levelID){
        // If empty tree
        if (root == null){
            test.push([levelID,counter]);
            return;
        }
        ...
        // switched.
        printOddNodes(root.left, !isOdd,counter,levelID);
        printOddNodes(root.right, !isOdd),counter,levelID;
}
...
function makeCircle(root,parent,side){
    if(root===parent){
        ...
        if(root == parent.left){
            ...
            circle.linewidth = 5;
            if(root.id == 1){
                circle.position.x = x1/(1 + Math.floor(Math.log2(test.length)));
            }
