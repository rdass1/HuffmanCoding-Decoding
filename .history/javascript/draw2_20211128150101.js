...
function makeCircle(root,parent,side){
    if(root===parent){
        ...
        if(root == parent.left){
            ...
            var extraOffset = 0;
            if(root.id % 2 != 0){
                extraOffset = x1/(1 + Math.floor(Math.log2(test.length)););
            }
            if(root.id % 4 == 0){
                extraOffset = Circle.radius * -6;
            }

            var circle = two.makeCircle(x1-Circle.offsetX-Circle.radius+extraOffset,y1+Circle.offsetY+Circle.radius,Circle.radius);
